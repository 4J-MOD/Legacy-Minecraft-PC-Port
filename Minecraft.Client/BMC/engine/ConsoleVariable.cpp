...
ConsoleVariable::~ConsoleVariable() {
    printf("Saving console variables...\n");
}
...
void ConsoleVariable::ClearVariable(const char* name) {
    std::shared_ptr<Config> conf = BMC::GetInstance().GetConfig();
    auto var = Get(name);
    if (var != nullptr) {
...
void ConsoleVariable::SaveToFile(const char* filename) {
    // This function can be implemented to save console variables to a specific file if needed
}

void ConsoleVariable::LoadFromFile(const char* filename) {
    // This function can be implemented to load console variables from a specific file if needed
}
} // namespace Ship
